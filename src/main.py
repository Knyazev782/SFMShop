...
create_user = User
user = create_user("Иван Иванов", "ivan@test.com")

create_product = Product
product_laptop = create_product("Ноутбук", 1, 50000)
product_mouse = create_product("Mouse", 1500, 2)

create_order = Order

# order = create_order(user, [product_laptop, product_mouse])

# calculate = order.calculate_total()
...
#
#
# order = Order(user, 'laptop', 1, 50000, 'Ivan')
# print(order)
...
# 2. Недостаточно товара
try:
    p2 = Product('mouse', 1500, 10)
    p2.sell(100)
...
# 4. Ловим через базовое исключение
try:
    p3 = Product('monitor', -100, 5)
except SFMShopException as e:
    print(f"Общая ошибка SFMShop: {type(e).__name__} - {e}")